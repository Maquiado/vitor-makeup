...
window.marcarRealizado = function(id) {
  db.collection("agendamentos").doc(id).get().then(doc => {
    if (doc.exists) {
      ...
      db.collection("agendamentos").doc(id).update({ status: "realizado" })
        .then(() => {
          ...
          // Adiciona estrela ao cliente
          const clienteRef = db.collection("clientes").doc(telefone);
          clienteRef.get().then(snapshot => {
            if (snapshot.exists) {
              clienteRef.update({
                estrelas: firebase.firestore.FieldValue.increment(1)
              });
            } else {
              clienteRef.set({
                nome: nome,
    ...
    showToast("Erro ao buscar agendamento: " + error.message, "bg-red-600");
  });
};
